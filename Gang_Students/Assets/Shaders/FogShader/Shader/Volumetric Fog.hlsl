/**
 * \brief Generuje losow¹ wartoœæ w zakresie [0, 1] na podstawie podanej wartoœci ziarna (Seed).
 *
 * Wykorzystuje funkcje sinusa, mno¿enie przez sta³e wartoœci oraz funkcjê frac.
 *
 * \param Seed Wartoœæ ziarna u¿ywana do generowania losowej wartoœci.
 * \return Wygenerowana losowa wartoœæ.
 */
...
/**
 * \brief Generuje próbkê mg³y na podstawie podanych parametrów.
 *
 * \param NoiseTexture Tekstura 3D zawieraj¹ca szum, u¿ywana do generowania wartoœci szumu.
 * \param Noise2DTexture Tekstura 2D zawieraj¹ca szum, u¿ywana do generowania wartoœci szumu.
 * \param Noise2DScale Skala aplikowana do pozycji na teksturze 2D w celu wygenerowania wartoœci szumu.
 * \param Samples Liczba próbek mg³y, które bêd¹ generowane wzd³u¿ widoku.
 * \param MeshDistance Odleg³oœæ, na jak¹ próbki mg³y s¹ generowane od siatki obiektu.
 * \param StartingHeight Pocz¹tkowa wysokoœæ mg³y.
 * \param OverallHeight Ca³kowita wysokoœæ mg³y.
 * \param randomness Wartoœæ losowa u¿ywana do wprowadzenia przypadkowoœci w generowanych próbkach.
 * \param Size Skalowanie pozycji próbek mg³y.
 * \param Threshold Próg wartoœci szumu, powy¿ej którego mg³a bêdzie generowana.
 * \param Multiplier Mno¿nik wp³ywaj¹cy na si³ê generowanej mg³y.
 * \param MaxDistance Maksymalna odleg³oœæ, na jak¹ próbki mg³y mog¹ siê rozprzestrzeniaæ.
 * \param Position Aktualna pozycja kamery/widza.
 * \param View Wektor kierunku widoku kamery/widza.
 * \param[out] Fog Wynikowa wartoœæ mg³y.
 */
